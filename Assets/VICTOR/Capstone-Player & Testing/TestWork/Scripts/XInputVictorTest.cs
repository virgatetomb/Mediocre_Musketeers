...
public class XInputVictorTest : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Mathf.Abs(state.ThumbSticks.Left.X) > deadZone)
            currentLoc.x += state.ThumbSticks.Left.X * moveSpeed * Time.deltaTime;
        if (Mathf.Abs(state.ThumbSticks.Left.Y) > deadZone)
            currentLoc.z += state.ThumbSticks.Left.Y * moveSpeed * Time.deltaTime;
        transform.Translate(currentLoc);
